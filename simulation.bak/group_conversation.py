...
GROUP_EXCHANGES = 6  # total turns across all participants
...
async def run_group_conversation(
    ...
    for char in characters:
        ...
        # Build prompt with awareness of the full group
        others = [c for c in characters if c.id != char.id]
        base = build_system_prompt(char, others[0], location, db, sim_day, inception)
        # Append group context
        group_note = (
    ...
    for turn in range(GROUP_EXCHANGES):
