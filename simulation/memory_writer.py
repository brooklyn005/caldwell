    ...
    for char, other in [(char_a, char_b), (char_b, char_a)]:
        ...
        try:
            ...
            if memory_text and memory_text.strip() not in ("...", ""):
                weight = _score_memory_weight(memory_text, transcript)
                write_memory(
                    character=char,
...
def _score_memory_weight(memory_text: str, transcript: str) -> float:
    """
    Assign emotional weight based on content.
    Higher weight = more significant, stays in prompt longer.
    """
    text = memory_text.lower()
    trans = transcript.lower()

    # High significance markers
    HIGH = [
        "died", "death", "dead", "born", "birth", "left", "gone", "never",
        "first time", "i decided", "i realized", "changed", "trust", "afraid",
        "love", "hate", "betrayed", "lied", "hit", "hurt", "blood", "cried",
        "witnessed", "saw something", "came out", "private", "alone",
    ]
    # Medium significance
    MEDIUM = [
        "argued", "disagreed", "angry", "said no", "refused", "asked me",
        "told me", "warned", "reminded", "remembered", "felt", "understood",
        "confused", "worried", "hungry", "rule", "agreed", "decided",
    ]

    high_count = sum(1 for w in HIGH if w in text or w in trans)
    med_count = sum(1 for w in MEDIUM if w in text)
    ...
    if high_count >= 2:
        ...
        return 0.55  # still above old 0.6 floor for truly mundane
